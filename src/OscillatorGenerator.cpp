...
void OscillatorGenerator::generate(double* pYn, double* pYqn) {
	...
	case sine:
		fOutSample = LinInterp::dLinTerp(0, 1, m_SinArray[nReadIndex], m_SinArray[nReadIndexNext], fFrac); // interpolate frac value 
		fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_SinArray[nQuadPhaseReadIndex], m_SinArray[nQuadPhaseReadIndexNext], fFrac);
		break;
	case saw:
		if (selectedMode == normal) {
			fOutSample = LinInterp::dLinTerp(0, 1, m_SawtoothArray[nReadIndex], m_SawtoothArray[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_SawtoothArray[nQuadPhaseReadIndex], m_SawtoothArray[nQuadPhaseReadIndexNext], fFrac);
		}
		else {
			fOutSample = LinInterp::dLinTerp(0, 1, m_SawtoothArray_BL5[nReadIndex], m_SawtoothArray_BL5[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_SawtoothArray_BL5[nQuadPhaseReadIndex], m_SawtoothArray_BL5[nQuadPhaseReadIndexNext], fFrac);
		}
		break;
	case tri:
		if (selectedMode == normal) {
			fOutSample = LinInterp::dLinTerp(0, 1, m_TriangleArray[nReadIndex], m_TriangleArray[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_TriangleArray[nQuadPhaseReadIndex], m_TriangleArray[nQuadPhaseReadIndexNext], fFrac);
		}
		else {
			fOutSample = LinInterp::dLinTerp(0, 1, m_TriangleArray_BL5[nReadIndex], m_TriangleArray_BL5[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_TriangleArray_BL5[nQuadPhaseReadIndex], m_TriangleArray_BL5[nQuadPhaseReadIndexNext], fFrac);
		}
		break;
	case square:
		if (selectedMode == normal) {
			fOutSample = LinInterp::dLinTerp(0, 1, m_SquareArray[nReadIndex], m_SquareArray[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_SquareArray[nQuadPhaseReadIndex], m_SquareArray[nQuadPhaseReadIndexNext], fFrac);
		}
		else {
			fOutSample = LinInterp::dLinTerp(0, 1, m_SquareArray_BL5[nReadIndex], m_SquareArray_BL5[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_SquareArray_BL5[nQuadPhaseReadIndex], m_SquareArray_BL5[nQuadPhaseReadIndexNext], fFrac);
		}
		break;
	default:
		fOutSample = LinInterp::dLinTerp(0, 1, m_SinArray[nReadIndex], m_SinArray[nReadIndexNext], fFrac);
		fQuadPhaseOutSample = LinInterp::dLinTerp(0, 1, m_SinArray[nQuadPhaseReadIndex], m_SinArray[nQuadPhaseReadIndexNext], fFrac);
		break;
	}
